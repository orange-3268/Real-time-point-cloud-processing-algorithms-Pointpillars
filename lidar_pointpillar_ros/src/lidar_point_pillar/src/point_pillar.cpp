...
TRT::TRT(std::string modelFile, cudaStream_t stream):stream_(stream)
{
  ...
  if (!trtCache.is_open())
  {
    ...
    {
        std::cout << ": failed to parse onnx model file, please check the onnx version and trt support op!"
                  << std::endl;
        // exit(-1);
    }
    ...
    if (engine_ == nullptr)
    {
      std::cout << ": engine init null!" << std::endl;
      // exit(-1);
    }
    ...
    if (runtime == nullptr) {	  
      ...
      std::cout << ": runtime null!" << std::endl;
        // exit(-1);
    }
...
int PointPillar::doinfer(void*points_data, unsigned int points_size, std::vector<Bndbox> &nms_pred)
{
  ...
  float totalTime = generateVoxelsTime + generateFeaturesTime + doinferTime + doPostprocessCudaTime;
  std::cout<<"TIME: totalTime: "<< totalTime<<"  ms"<<std::endl;
#endif
  return 0;
}

void PointPillar::detect(pcl::PointCloud<pcl::PointXYZ>::Ptr &input,std::vector<Bndbox> &nms_pred)
{
  //1. load point cloud 
  std::cout << "<<<<<<<<<<<" <<std::endl;
  ...
  for(std::size_t i = 0; i < points_size;i++){
    points[i * 4 + 0] = input->points[i].x;
    points[i * 4 + 1] = input->points[i].y;
    points[i * 4 + 2] = input->points[i].z;
    points[i * 4 + 3] = 0.0f;
  }
  //2.infer
  ...
  checkCudaErrors(cudaDeviceSynchronize());
  int success = doinfer(points_data, points_size, nms_pred);
